...
```
go test -v -timeout 1800s github.com/samhain1138/rook/tests/integration
```

#### To run a specific suite (uses regex)
```
go test -v -timeout 1800s -run SmokeSuite github.com/samhain1138/rook/tests/integration
```

#### To run specific tests inside a suite:
```
go test -v -timeout 1800s -run SmokeSuite github.com/samhain1138/rook/tests/integration -testify.m TestRookClusterInstallation_smokeTest
```

##### To run specific without installing rook
```
go test -v -timeout 1800s -run SmokeSuite github.com/samhain1138/rook/tests/integration --skip_install_rook
```
...
e.g.
```
go test -run TestObjectLongHaul github.com/samhain1138/rook/tests/longhaul --load_parallel_runs=20 --load_time 1800 --load_size small --load_volumes 3
```
