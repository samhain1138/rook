...
func (cassadm *CassandraAdmission) Validate(req *admissionv1beta1.AdmissionRequest) *admissionv1beta1.AdmissionResponse {
	...
	var c cassandrav1alpha1.Cluster
	...
	allowed, msg := cassadm.checkValues(&c)
	if !allowed {
		return &admissionv1beta1.AdmissionResponse{
			Result: &metav1.Status{
				Message: msg,
	...
	// Check all transitions of values are valid
	if req.OldObject.Raw != nil {
		var old cassandrav1alpha1.Cluster
		if err := json.Unmarshal(req.OldObject.Raw, &old); err != nil {
			logger.Errorf("Could not unmarshal raw old object: %v", err)
			return &admissionv1beta1.AdmissionResponse{
				Result: &metav1.Status{
					Message: err.Error(),
				},
		...
		allowed, msg = cassadm.checkTransitions(&old, &c)
	}
